...
func (a *App) GetPublicLists(c echo.Context) error {
	...
	lists, err := a.mailviewCore(c).GetLists(models.ListTypePublic, models.ListStatusActive, true, nil)
	if err != nil {
...
func (a *App) ViewCampaignMessage(c echo.Context) error {
	...
	camp, err := a.mailviewCore(c).GetCampaign(0, campUUID, "")
	if err != nil {
	...
	sub, err := a.mailviewCore(c).GetSubscriber(0, subUUID, "")
	if err != nil {
...
func (a *App) SubscriptionPage(c echo.Context) error {
	...
	s, err := a.mailviewCore(c).GetSubscriber(0, subUUID, "")
	if err != nil {
		return c.Render(http.StatusInternalServerError, tplMessage,
	...
	if a.cfg.Privacy.AllowPreferences {
		...
		subs, err := a.mailviewCore(c).GetSubscriptions(0, subUUID, false)
		if err != nil {
			return echo.NewHTTPError(http.StatusBadRequest, a.i18n.T("public.errorFetchingLists"))
		}
...
func (a *App) SubscriptionPrefs(c echo.Context) error {
	...
	if !req.Manage || blocklist {
		if err := a.mailviewCore(c).UnsubscribeByCampaign(subUUID, campUUID, blocklist); err != nil {
			return c.Render(http.StatusInternalServerError, tplMessage,
	...
	if _, err := a.mailviewCore(c).UpdateSubscriber(sub.ID, sub); err != nil {
		return c.Render(http.StatusInternalServerError, tplMessage,
			makeMsgTpl(a.i18n.T("public.errorTitle"), "", a.i18n.T("public.errorProcessingRequest")))
	}
	...
	if err := a.mailviewCore(c).UnsubscribeLists([]int{sub.ID}, nil, unsubUUIDs); err != nil {
		return c.Render(http.StatusInternalServerError, tplMessage,
			makeMsgTpl(a.i18n.T("public.errorTitle"), "", a.i18n.T("public.errorProcessingRequest")))
...
func (a *App) OptinPage(c echo.Context) error {
	...
	lists, err := a.mailviewCore(c).GetSubscriberLists(0, subUUID, nil, req.ListUUIDs, models.SubscriptionStatusUnconfirmed, "")
	if err != nil {
		return c.Render(http.StatusInternalServerError, tplMessage,
...
func (a *App) confirmOptinSubscription(c echo.Context, subUUID string, listUUIDs []string, lists []models.List) error {
	...
	if err := a.mailviewCore(c).ConfirmOptionSubscription(subUUID, listUUIDs, meta); err != nil {
...
func (a *App) LinkRedirect(c echo.Context) error {
	...
	if a.cfg.Privacy.DisableTracking {
		url, err := a.mailviewCore(c).GetLinkURL(linkUUID)
		if err != nil {
	...
	url, err := a.mailviewCore(c).RegisterCampaignLinkClick(linkUUID, campUUID, subUUID)
	if err != nil {
		e := err.(*echo.HTTPError)
		return c.Render(e.Code, tplMessage, makeMsgTpl(a.i18n.T("public.errorTitle"), "", e.Error()))
	}
...
func (a *App) RegisterCampaignView(c echo.Context) error {
	...
	if campUUID != dummyUUID && subUUID != dummyUUID {
		if err := a.mailviewCore(c).RegisterCampaignView(campUUID, subUUID); err != nil {
...
func (a *App) SelfExportSubscriberData(c echo.Context) error {
	...
	subUUID := c.Param("subUUID")
	data, b, err := a.exportSubscriberData(c, 0, subUUID, a.cfg.Privacy.Exportable)
	if err != nil {
...
func (a *App) WipeSubscriberData(c echo.Context) error {
	...
	subUUID := c.Param("subUUID")
	if err := a.mailviewCore(c).DeleteSubscribers(nil, []string{subUUID}); err != nil {
...
func (a *App) processSubForm(c echo.Context) (bool, error) {
	...
	listTypes, err := a.mailviewCore(c).GetListTypes(nil, req.FormListUUIDs)
	if err != nil {
	...
	_, hasOptin, err := a.mailviewCore(c).InsertSubscriber(models.Subscriber{
		Name:   req.Name,
		Email:  req.Email,
	...
	if e, ok := err.(*echo.HTTPError); ok && e.Code == http.StatusConflict {
		...
		sub, err := a.mailviewCore(c).GetSubscriber(0, "", req.Email)
		if err != nil {
			return false, err
		}

		// Update the subscriber's subscriptions in the DB.
		_, hasOptin, err := a.mailviewCore(c).UpdateSubscriberWithLists(sub.ID, sub, nil, listUUIDs, false, false, true, nil, true)
		if err == nil {
			return hasOptin, nil
		}
		lastErr = err
