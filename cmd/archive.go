...
func (a *App) GetCampaignArchives(c echo.Context) error {
	...
	camps, total, err := a.getCampaignArchives(c, pg.Offset, pg.Limit, false)
	if err != nil {
		return err
	}
...
func (a *App) GetCampaignArchivesFeed(c echo.Context) error {
	...
	// Get archives from the DB.
	camps, _, err := a.getCampaignArchives(c, pg.Offset, pg.Limit, showFullContent)
	if err != nil {
		return err
	}
...
func (a *App) CampaignArchivesPage(c echo.Context) error {
	// Get archives from the DB.
	pg := a.pg.NewFromURL(c.Request().URL.Query())
	out, total, err := a.getCampaignArchives(c, pg.Offset, pg.Limit, false)
	if err != nil {
		return err
	}
	pg.SetTotal(total)
...
func (a *App) CampaignArchivePage(c echo.Context) error {
	...
	pubCamp, err := a.mailviewCore(c).GetArchivedCampaign(0, uuid, slug)
...
func (a *App) CampaignArchivePageLatest(c echo.Context) error {
	...
	camps, _, err := a.getCampaignArchives(c, 0, 1, true)
	if err != nil {
		return err
	}
...
func (a *App) getCampaignArchives(c echo.Context, offset, limit int, renderBody bool) ([]campArchive, int, error) {
	pubCamps, total, err := a.mailviewCore(c).GetArchivedCampaigns(offset, limit)
	if err != nil {
